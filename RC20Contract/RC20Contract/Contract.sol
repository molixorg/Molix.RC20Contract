...
contract MolixToken is StandardToken, SafeMath {
    /*
     * Token meta data
     */
    string public constant name = "MolixToken";
    ...
    function MolixToken(address _icoContract) {
...
contract MolixIco is SafeMath {
    /*
     * ICO meta data
     */
    MolixToken public molixToken;
    ...
    // 101.000.000 MLC tokens
    uint public constant supplyLimit = 101000000000000000000000000;

    //  20000000 MLC is token for bountyOwner
    uint public constant bountyOwnersTokens = 20000000000000000000000000;

    // 1 ETH = 1600 MLC
    uint public constant PRICE = 1600;

    // BASE = 10^18
    uint constant BASE = 1000000000000000000;

    // 2018.07.05 00:00 UTC
    // founders' reward time
    uint public foundersRewardTime = 1530723600000;
    ...
    function MolixIco(address _icoManager, address _preIcoToken) {
        assert(_preIcoToken != 0x0);
        assert(_icoManager != 0x0);

        molixToken = new MolixToken(this);
    ...
    function importTokens(address _account) {
        ...
        if (preIcoBalance > 0) {
            molixToken.emitTokens(_account, preIcoBalance);
    ...
    function buyTokens(address _buyer) private {
        ...
        molixToken.emitTokens(_buyer, tokensToEmit);
    ...
    function burnTokens(address _from, uint _value) onlyManager notMigrated {
        molixToken.burnTokens(_from, _value);
    }
    ...
    function sendTokensToBountyOwner() onlyManager whenInitialized {
        ...
        molixToken.emitTokens(bountyOwner, bountyTokens);

        sentTokensToBountyOwner = true;
    }

    /// @dev Send tokens to founders. Can be sent only after molixToken.rewardTime() (2018.07.05 0:00 UTC)
    function sendTokensToFounders() onlyManager whenInitialized {
        ...
        molixToken.emitTokens(founder1, founderReward);
        molixToken.emitTokens(founder2, founderReward);
        molixToken.emitTokens(founder3, founderReward);
        molixToken.emitTokens(founder4, founderReward);
